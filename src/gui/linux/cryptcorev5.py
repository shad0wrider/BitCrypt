...
def enc(srcfile:str,password:str,ipcfile,folderpath:str):
    """
    **`srcfile`** - Path To File
    ...
    **`folderpath`** - Folder to save encrypted file in
    """
    ipcsocket = socket.socket(socket.AF_UNIX,socket.SOCK_DGRAM)
    ...
    try:
        ...
        if mixkey :
            ...
            if os.path.getsize(srcfile) < 212806066:
                ...
                #Small File Mode
                ipcsocket.sendto("info:small file mode".encode("utf-8"),ipcfile)
                ...
                #Large File Mode
                hmckey = os.urandom(64)
                ipcsocket.sendto("info:Large file mode".encode("utf-8"),ipcfile)
        ...
        ipcsocket.sendto(f"error: Backgroud Process \n{oa}".encode("utf-8"),ipcfile)
            

def dec(srcfile:str,passw:str,ipcfile,folderpath:str):
    """
    **`srcfile`** - Path To File
        ...
        if os.path.isfile(srcfile):
            ...
            if fchk ==0:
                ...
                try:
                    ...
                    ipcsocket.sendto("error:File Header is Corrupted :(\n Try Recovery Mode".encode('utf-8'),ipcfile)

                decryptfilesize = decfilesize
                mixkey = mixpass.passmixer(password=passw)[:32].encode('utf-8')
                ...
                if tmptopkey !=1:
                    ...
                    ipcsocket.sendto("error:Master Key Derivation Error".encode("utf-8"),ipcfile)
